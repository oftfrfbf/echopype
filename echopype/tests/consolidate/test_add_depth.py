...
@pytest.mark.parametrize("file, sonar_model, compute_Sv_kwargs", [
    (
        "ek60/NBP_B050N-D20180118-T090228.raw",
        "EK60",
        {}
    ),
    (
        "ek60/ncei-wcsd/Summer2017-D20170620-T021537.raw",
        "EK60",
        {}
    ),
    (
        "ek80/ncei-wcsd/SH1707/Reduced_D20170826-T205615.raw",
        "EK80",
        {"waveform_mode":"BB", "encode_mode":"complex"}
    ),
    (
        "ek80/ncei-wcsd/SH2106/EK80/Reduced_Hake-D20210701-T131621.raw",
        "EK80",
        {"waveform_mode":"CW", "encode_mode":"power"}
    )
])
def test_ek_depth_utils_dims(file, sonar_model, compute_Sv_kwargs, root_path):
    """
...
def test_ek_depth_utils_group_variable_NaNs_logger_warnings(caplog, ek80_path):
    ...
    # Open EK Raw file and Compute Sv
    ed = ep.open_raw(
        raw_file=ek80_path.joinpath("ncei-wcsd/SH2106/EK80/Reduced_Hake-D20210701-T131621.raw"),
        sonar_model="EK80"
    )
...
def test_add_depth_EK_with_platform_vertical_offsets(file, sonar_model, compute_Sv_kwargs, root_path):
...
def test_add_depth_EK_with_platform_angles(file, sonar_model, compute_Sv_kwargs, root_path):
...
def test_add_depth_EK_with_beam_angles(file, sonar_model, compute_Sv_kwargs, root_path):
    """Test `depth` values when using EK Beam angles to compute it."""
    # Open EK Raw file and Compute Sv
    ed = ep.open_raw(
        raw_file=root_path.joinpath(file),
        sonar_model=sonar_model
    )
    ds_Sv = ep.calibrate.compute_Sv(ed, **compute_Sv_kwargs)
...
@pytest.mark.integration
def test_add_depth_with_external_glider_depth_and_tilt_array(root_path):
    """
    Test add_depth with external glider depth offset and tilt array data.
    """
    # Open RAW
    ed = ep.open_raw(
        raw_file=root_path.joinpath("azfp/rutgers_glider_external_nc/18011107.01A"),
        xml_path=root_path.joinpath("azfp/rutgers_glider_external_nc/18011107.XML"),
        sonar_model="azfp"
    )

    # Open external glider dataset
    glider_ds = xr.open_dataset(
        root_path.joinpath("azfp/rutgers_glider_external_nc/ru32-20180109T0531-profile-sci-delayed-subset.nc"),
        engine="netcdf4"
    )
...
@pytest.mark.unit
def test_multi_dim_depth_offset_and_tilt_array_error(root_path):
    """
    ...
    # Open EK Raw file and Compute Sv
    ed = ep.open_raw(
        raw_file=root_path.joinpath("ek80/ncei-wcsd/SH2106/EK80/Reduced_Hake-D20210701-T131621.raw"),
        sonar_model="EK80"
    )
    ds_Sv = ep.calibrate.compute_Sv(ed, **{"waveform_mode":"CW", "encode_mode":"power"})
