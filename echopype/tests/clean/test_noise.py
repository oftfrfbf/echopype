...
def test_mask_functions_with_no_vertical_range_variables(range_var, ek60_path):
    ...
    ed = ep.open_raw(
        raw_file=ek60_path.joinpath("from_echopy", "JR230-D20091215-T121917.raw"),
        sonar_model="EK60"
    )
...
def test_index_binning_pool_Sv_values(chunk, func, ek60_path):
    ...
    # Open raw, calibrate, and add depth
    ed = ep.open_raw(
        raw_file=ek60_path.joinpath("from_echopy", "JR161-D20061118-T010645.raw"),
        sonar_model="EK60"
    )
    ds_Sv = ep.calibrate.compute_Sv(ed)
    ds_Sv = ep.consolidate.add_depth(ds_Sv)
...
def test_mask_attenuated_signal_against_echopy(chunk, ek60_path):
    ...
    echopy_attenuated_mask = xr.open_dataset(
        ek60_path.joinpath("from_echopy", "JR161-D20061118-T010645_echopy_attenuated_masks.zarr"),
        engine="zarr"
    )
...
def test_estimate_background_noise_upsampling(ek60_path):
    ...
    # Open Raw and Calibrate
    ed = ep.open_raw(
        raw_file=ek60_path.joinpath("ncei-wcsd", "Summer2017-D20170615-T190214.raw"),
        sonar_model="EK60"
    )
    ds_Sv = ep.calibrate.compute_Sv(ed)
